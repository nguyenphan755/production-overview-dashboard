...
const HeaderClockContext = createContext<Date | null>(null);

function HeaderClockProvider({ children }: { children: ReactNode }) {
  ...
  return <HeaderClockContext.Provider value={value}>{children}</HeaderClockContext.Provider>;
}

function HeaderClockDesktopSlot() {
  const t = useContext(HeaderClockContext);
  if (!t) return null;
  return (
    <div className="text-left">
      <div className="flex items-center gap-2 mb-1">
        <Clock className="w-4 h-4 text-[#34E7F8]" />
        <div className="text-3xl text-[#34E7F8] tracking-wider font-mono">{formatTime(t)}</div>
      </div>
      <div className="text-white/60 text-sm">{formatDate(t)}</div>
    </div>
  );
}

function HeaderClockMobileSlot() {
  const t = useContext(HeaderClockContext);
  if (!t) return null;
  return (
    <div className="text-right">
      <div className="flex items-center justify-end gap-2">
        <Clock className="w-4 h-4 text-[#34E7F8]" />
        <div className="text-lg text-[#34E7F8] tracking-wider font-mono">{formatTime(t)}</div>
      </div>
      <div className="text-white/60 text-xs">{formatDate(t)}</div>
    </div>
  );
}

const DesktopHeaderBranding = memo(function DesktopHeaderBranding() {
  return (
...
const MobileHeaderBranding = memo(function MobileHeaderBranding() {
  return (
    <div className="flex items-center gap-2">
...
export const CompanyHeader = memo(function CompanyHeader({ onLogout }: CompanyHeaderProps) {
  return (
    <HeaderClockProvider>
      <>
